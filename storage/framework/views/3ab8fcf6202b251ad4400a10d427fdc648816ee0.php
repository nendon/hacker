...
<div class="row">
  <div class="col-md-12">
    <div class="container" style="background-color:#fff;padding-bottom:25px;">
        <div class="col-md-12">
            <h3><?php echo e($datalesson->title); ?></h3>
            <hr>
            <img src="<?php echo e($datalesson->cover); ?>" class="img-responsive">
        </div>
        ...
        <div class="col-md-12">
            ...
            <div class="content-reload">
                <?php $__currentLoopData = $datacomment; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="col-md-12" style="margin-bottom:30px;" id="row<?php echo e($comment->id); ?>">
                    <strong> <?php if($comment->desc == 0): ?>  <?php echo e($comment->username); ?> <?php else: ?>  <?php echo e($comment->contriname); ?> <?php endif; ?> </strong> pada <strong><?= date('d/m/Y',strtotime($comment->created_at)) ?></strong>
					<strong style="color:#ff5e10;"><?php if($comment->desc == 0): ?>  User <?php else: ?>  Contributor <?php endif; ?></strong>
					<div class="col-md-12" style="margin-top:10px;padding-left:5%; white-space:pre-line;">
                            <?php echo e($comment->body); ?>

                    <?php if(!empty($comment->images)) { ?>
                            <a id="firstlink" data-gall="myGallery" class="venobox" data-vbtype="iframe" href="<?php echo e(asset($comment->images)); ?>"><img src="<?php echo e(asset($comment->images)); ?>" alt="image alt" style="height:50px; width:50px; margin-left: 15px; margin-bottom: 20px;"/></a>
                    <?php } ?>
					</div>
            ...
            <?php if($child->desc == 0){ ?>
              <?php if(!empty($child->ava)): ?>
							<img class="user-photo" src="<?php echo e($child->ava); ?>"height="40px" width="40px" style="object-fit:scale-down;border-radius: 100%;margin-bottom:10px;">
              <?php else: ?>
							<img class="user-photo" src="<?php echo e(asset($child->slug)); ?>"height="40px" width="40px" style="object-fit:scale-down;border-radius: 100%;margin-bottom:10px;">
              <?php endif; ?>
            <?php }else{ ?>
              <?php if(!empty($child->avatar)): ?>
              <img class="user-photo" src="<?php echo e($child->avatar); ?>"height="40px" width="40px" style="object-fit:scale-down;border-radius: 100%;margin-bottom:10px;">

              <?php else: ?>
              <img class="user-photo" src="<?php echo e(asset($child->slg)); ?>"height="40px" width="40px" style="object-fit:scale-down;border-radius: 100%;margin-bottom:10px;">
            ...
            <?php } ?>
								...
								<strong>
									<?php if($child->desc == 0){ ?>
										<?php echo e($child->username); ?>

									<?php }else{ ?>
										<?php echo e($child->contriname); ?>

									<?php } ?>
								</strong> pada <strong><?= date('d/m/Y',strtotime($child->created_at)) ?></strong>
								<strong style="color:#ff5e10;"><?php if($child->desc == 0): ?>  User <?php else: ?>  Contributor <?php endif; ?></strong>
								<div class="col-md-12" style="margin-top:10px;margin-bottom:10px;padding-left:5%; white-space: pre-line">
                                    <?php echo e($child->body); ?>

                                    <?php if($child->images != null) { ?>
                                        <a id="firstlink" data-gall="myGallery" class="venobox" data-vbtype="iframe" href="<?php echo e(asset($child->images)); ?>"><img src="<?php echo e(asset($child->images)); ?>" alt="image alt" style="height:50px; width:50px; margin-left: 15px; margin-bottom: 20px;"/></a>
                                <?php } ?>
								</div>
								<div class="clearfix"></div>
						</div>
                    ...
                    <div class="col-md-12" id="balas<?php echo e($comment->id); ?>" style="padding-top:10px;padding-left:5%; ">
                        <a href="javascript:void(0)" class="btn btn-info pull-right" onclick="formbalas(<?php echo e($comment->id); ?>)" style="border-radius:3px;">Balas</a>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function dobalas(comment_id){
        ...
        var bootcamp_id = '<?php echo e($comment->bootcamp_id); ?>';
        var member_id = '<?php echo e($comment->member_id); ?>';
        ...
        if (isi_balas == '') {
          ...
          $.ajax({
              type    :"POST",
              url     :'<?php echo e(url("contributor/bootcamp/comments/postcomment")); ?>',
              data    : dataform,
              dataType : 'json',
              contentType: false,
              processData: false,
